...
- **路由守卫**: 所有学习相关页面添加 `requiresAuth: true`
- **自动跳转**: 未登录用户自动跳转到登录页
...
| 场景 | 音效 | 实现 |
|------|------|------|
| 答对 | "excellent" 语音 | Web Speech API |
| 答错 | 下降音调 | Web Audio API |
...
// 未达标时禁用打卡按钮
if (todayLearnedWords < dailyGoal) {
  // 显示提示: "今日已学 X 个单词，需完成 Y 个才能打卡"
}
```
...
**问题**: 原来只有4关，通关后就没有新关卡了。

**解决方案**: 实现动态关卡生成系统！

| 难度等级 | 关卡名称 | 单词数 | 时间限制 | 错误次数 | XP奖励 |
|----------|----------|--------|----------|----------|--------|
| 简单 | 新手训练 #N | 5个 | 无限制 | 2次 | 10+ |
| 中等 | 速度挑战 #N | 10个 | 60秒 | 2次 | 20+ |
| 困难 | 完美挑战 #N | 15个 | 90秒 | 0次！ | 35+ |
| 专家 | 终极挑战 #N | 20个 | 120秒 | 1次 | 50+ |
| 传说 | 传奇挑战 #N | 30个 | 150秒 | 1次 | 80+ |
...
**代码位置**: [challenge.controller.ts](server/src/controllers/challenge.controller.ts)

### 7. 例句朗读功能

**功能**: 学习单词时，例句旁边显示喇叭按钮，点击可朗读例句。
...
**代码位置**: [Learning.vue](client/src/views/Learning.vue)

```vue
<!-- 例句显示区 -->
<div class="example-content">
  <p class="example-sentence">{{ example.sentence }}</p>
  <button class="example-audio-btn" @click="playExampleAudio">
    🔊
  </button>
</div>
```